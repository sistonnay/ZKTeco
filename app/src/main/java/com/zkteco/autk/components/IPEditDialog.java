 ...
 */
public abstract class IPEditDialog {

    private static final String TAG = Utils.TAG + "#" + IPEditDialog.class.getSimpleName();
    ...
    public IPEditDialog(Context context, String title, int inputType) {
        mContext = context;
        mTitle = title;
    ...
    public IPEditDialog(Context context, int title, int inputType) {
    ...
    public void show() {
        final View root = LayoutInflater.from(mContext).inflate(R.layout.ip_et_ly, null);
        final EditText ip_et = (EditText) root.findViewById(R.id.et_ip);
        ip_et.setInputType(mInputType);

        final EditText port_et = (EditText) root.findViewById(R.id.et_port);
        port_et.setInputType(mInputType);
        ...
        mDialogBuilder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                onDialogOK(ip_et.getText().toString(), port_et.getText().toString());
                dialog.dismiss();
            }
        });
        if (!cancelDisabled) {
    ...
    public abstract void onDialogOK(String ip, String port);
}